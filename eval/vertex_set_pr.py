...
def precision_recall_from_vertex_set(pred_vertices, gt_vertices,threshold = 3):
    #pred_vertices,gt_vertices: ndarray(n,2)
    #threshold:pixels 距离小于阈值的预测点被认为是正确的
    count = 0
...
def recall_from_vertex_set(pred_vertices, gt_vertices,threshold = 3):
    #pred_vertices,gt_vertices: ndarray(n,2)
    # threshold pixels 距离小于阈值的预测点被认为是正确的
    count = 0
    for point in gt_vertices:
    ...
    recall = count/len(gt_vertices) if len(gt_vertices) else 0
    return recall
def precision_from_vertex_set(pred_vertices, gt_vertices,threshold = 3):
    #pred_vertices,gt_vertices: ndarray(n,2)
    #threshold pixels 距离小于阈值的预测点被认为是正确的
    count =0
    for point in pred_vertices:
...
if __name__ == "__main__":
    ...
    threshold = 3
    recall = recall_from_vertex_set(pred_vertices, gt_vertices, threshold)
    precision = precision_from_vertex_set(pred_vertices, gt_vertices, threshold)
